...
export function buildSettingsPanel(deps) {
    ...
    async function showSnapshotPicker(snaps) {
        // Build a simple chooser as HTML buttons inside a popup.
        ...
        // We can't easily render interactive buttons inside callPopup on all
        // builds, so use a numeric prompt: ask which number to export.
        const ctx = getContext();
        let choiceStr = null;
        try {
            if (ctx?.Popup?.show?.input) {
                choiceStr = await ctx.Popup.show.input(
                    ...
                    `Type the NUMBER of the snapshot to export:\n\n${rows}`,
                );
                ...
                choiceStr = await ctx.callPopup(
                    ...
                    + `<pre style="white-space:pre-wrap;text-align:left;">${escapeHtmlLocal(rows)}</pre>`
                    + `<input type="number" min="1" max="${snaps.length}" id="acr_snap_choice" class="text_pole" value="1">`,
                    'input',
                );
                // Some builds return the input value; some need manual read.
  ...
  <div class="inline-drawer-content">
    ...
    <div id="acr_anime_box" class="acr-subbox" style="display:none;">
      ...
      <label class="acr-row">Autonomous snapshot threshold: <span id="acr_anime_snap_val"></span>
        <input type="range" id="acr_anime_snap_threshold" min="120000" max="190000" step="1000">
      </label>
    ...
    <label class="acr-row">Trigger Threshold (tokens): <span id="acr_threshold_val"></span>
      <input type="range" id="acr_threshold" min="120000" max="190000" step="1000">
    </label>
