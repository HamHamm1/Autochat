...
import {
    ...
    removeSnapshot, setLastRollover, getStore, persist,
} from './storage.js';
...
export class RolloverEngine {
    ...
    async _commit({ summary, usedProfile, usedLabel, cfg, ctxData, startTokens, source }) {
        ...
        // Show WHAT was summarized AND let the user edit it before continuing.
        // Editing re-injects the corrected memory and updates this snapshot.
        await this.editInjectedMemory(
            summary,
            `Profile: ${usedLabel} · trimmed ${removed.length}, kept ${newChat.length}`,
        );
    ...
    /**
     * Show the canonical memory in an EDITABLE popup. If the user changes it and
     * saves, the edited text is re-injected and the latest snapshot's summary is
     * updated (so rollback/export reflect the edit). Doubles as the post-rollover
     * "here's what was summarized" view. Best-effort; never throws.
     *
     * @param {string|null} initialText  Text to seed; falls back to latest snapshot.
     * @param {string} infoLine          Optional context line shown above the editor.
     */
    async editInjectedMemory(initialText = null, infoLine = '') {
        const ctx = this.context;

        let current = initialText;
        if (current == null) {
            current = getLatestSnapshot()?.summary || '';
        }
        if (!current) {
            this._notify(getConfig(), 'No summary to edit yet.', 'warning');
            return;
        }

        const desc = (infoLine ? `${infoLine}<br><br>` : '')
            + 'Edit the summary below, then Save to re-inject it. '
            + 'Cancel keeps it unchanged.';

        let edited = null;
        try {
            if (ctx?.Popup?.show?.input) {
                // rows makes it a multi-line textarea on builds that support it.
                edited = await ctx.Popup.show.input(
                    'Summary (editable)', desc, current, { rows: 16 },
                );
            } else if (typeof ctx?.callPopup === 'function') {
                const html = `<h3>Summary (editable)</h3><p>${desc}</p>`
                    + `<textarea id="acr_mem_edit" class="text_pole" `
                    + `style="width:100%;min-height:45vh;">`
                    + `${this._escapeHtml(current)}</textarea>`;
                const r = await ctx.callPopup(html, 'confirm');
                const field = document.getElementById('acr_mem_edit');
                edited = (r && field) ? field.value : null;
            }
        } catch (e) {
            this.log('debug', 'Edit memory popup skipped:', e?.message);
            return;
        }

        // Cancelled, empty, or unchanged → leave the injected memory as-is.
        if (edited == null || edited === false) return;
        const text = String(edited).trim();
        if (!text || text === String(current).trim()) return;

        // Re-inject the corrected memory.
        this._injectCanonicalMemory(text);

        // Update the latest snapshot so rollback/export carry the edit.
        try {
            const snap = getLatestSnapshot();
            if (snap) snap.summary = text;
            if (typeof ctx.saveSettings === 'function') await ctx.saveSettings();
            else persist();
        } catch (e) {
            this.log('debug', 'Snapshot summary update failed:', e?.message);
        }

        this.tokenMonitor?.requestRecount('memory-edited');
        this._notify(getConfig(), 'Canonical memory updated.', 'success');
    }
    ...
    async _refreshChatUI() {
        const ctx = this.context;
        try {
            // 1. Persist the trimmed chat to disk FIRST and wait for it.
            if (typeof ctx.saveChat === 'function') {
                await ctx.saveChat();
            }

            // 2. Refresh the visible messages from the IN-MEMORY chat array,
            //    WITHOUT re-fetching from the server. reloadCurrentChat() does a
            //    server round-trip that, on high-latency web hosts, can read a
            //    stale (pre-trim) copy and make the trimmed messages REAPPEAR
            //    (chat stays at its old length). printMessages() renders straight
            //    from ctx.chat, which we already trimmed in place — so the UI
            //    always matches memory and no stale data can sneak back in.
            let refreshed = false;
            try {
                ...
                this.log('debug', 'printMessages refresh failed:', e?.message);
            }

            // 3. Last-resort fallback only if no in-memory renderer is available.
            if (!refreshed && typeof ctx.reloadCurrentChat === 'function') {
